...
class Triangle(var fx: Float, var fy: Float, var sx: Float, var sy: Float, var tx: Float, var ty: Float, var paint: Paint, var isSelected: Boolean = false) {
    ...
    private fun getPivotPoints(): MutableList<PivotPoint> {
        return mutableListOf(
            ...
            PivotPoint(tx, ty, PAINT.PIVOT_POINT_PAINT, this),
        )
    }

    fun getDrawableLines(): List<Line> {
        return listOf(
            Line(fx, fy, sx, sy, paint, this),
            Line(fx, fy, tx, ty, paint, this),
            Line(sx, sy, tx, ty, paint, this)
        )
    }
    ...
    fun update() {
        ...
        sx = pivotPointsList[1].x
        sy = pivotPointsList[1].y
        tx = pivotPointsList[2].x
        ty = pivotPointsList[2].y
        drawableLinesList = getDrawableLines()
    }
}

fun Canvas.drawTriangle(triangle: Triangle) {
    val paint = if (triangle.isSelected) PAINT.GREEN else triangle.paint
    triangle.getDrawableLines().forEach {
